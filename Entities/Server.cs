...
namespace Valkyrja.entities
{
	public class Server
	{
		...
		public List<CategoryMemberRole> CategoryMemberRoles;
		...
		public async Task ReloadConfig(ValkyrjaClient client, ServerContext dbContext, Dictionary<string, Command> allCommands)
		{
			...
			this.CategoryMemberRoles = dbContext.CategoryMemberRoles.AsQueryable().Where(c => c.ServerId == this.Id).ToList();
		...
		public async Task<bool> HandleHttpException(HttpException exception, string helptext = "")
		{
			...
			if( (int)exception.HttpCode >= 500 )
				logMsg = "DiscordPoop";
			else if( (int)exception.HttpCode == 404 || (int)exception.HttpCode == 400 )
				logMsg = "";
